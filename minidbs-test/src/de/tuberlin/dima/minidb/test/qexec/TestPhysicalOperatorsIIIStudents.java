...
import de.tuberlin.dima.minidb.qexec.solution.MergeJoinOperatorImpl;
...
public class TestPhysicalOperatorsIIIStudents {
	 ...
	 */
	private static final String CONFIG_FILE_NAME = TestPhysicalOperatorsIIIStudents.class.getResource("/config.xml").getPath();
	
	/**
	 * The path to the file with the catalogue.
	 */
	private static final String CATALOGUE_FILE_NAME = TestPhysicalOperatorsIIIStudents.class.getClass().getResource("/catalogue.xml").getPath();
...
//		consoleHandler.setFormatter(new ConsoleMessageFormatter());
	...
	public void testSimpleCount() throws Exception
	{
		...
		BTreeIndex referenceIndex = AbstractExtensionFactory.getExtensionFactory().createBTreeIndex(id.getSchema(), pool, id.getResourceId());
		IndexLookupOperator referenceIndexScan = new IndexLookupOperatorImpl(
				referenceIndex, DataType.intType().getNullValue(), new IntField(Integer.MAX_VALUE), true, true);
		
		GroupByOperator referenceGroupBy = new GroupByOperatorImpl(referenceIndexScan,
				new int[] {}, // no grouping columns
				new int[] {0}, // the only column (RID column) is aggregated once
				new OutputColumn.AggregationType[] { OutputColumn.AggregationType.COUNT }, // count the rids
				new DataType[] { DataType.ridType() }, // type is RID
		...
		exercisePlanTests(testGroupBy, referenceGroupBy);
	}
	...
	public void testGroupCustomers() throws Exception
	{
		...
		// set up the reference plan
		TableScanOperator referenceTableCustomerScan = new TableScanOperatorImpl(
				bufferPool, custTable.getResourceManager(), null, customerColumns, 
				custTable.getResourceId(), 150);
		
		SortOperator referenceSort = new SortOperatorImpl(referenceTableCustomerScan,
				this.theInstance.getQueryHeap(), sortType,
				(int) custTable.getStatistics().getCardinality(), sortAndGroupCols, new boolean[] {false});
		
		GroupByOperator referenceGroupBy = new GroupByOperatorImpl(referenceSort,
				sortAndGroupCols, aggColIndices, aggs, aggTypes,
				groupOutput, aggOutput);
		
		TableScanOperator referenceNationScan = new TableScanOperatorImpl(
				bufferPool, nationTable.getResourceManager(), null, nationColumns,
				nationTable.getResourceId(), 10);
		
		NestedLoopJoinOperator referenceNlJoin = new NestedLoopJoinOperatorImpl(
				referenceGroupBy, referenceNationScan, joinPred, leftHandOutput, rightHandOutput);
		
		SortOperator referenceFinalSort = new SortOperatorImpl(referenceNlJoin,
				this.theInstance.getQueryHeap(), finalSortType,
				(int) nationTable.getStatistics().getCardinality(), finalSortColumns, finalSortOrder);
		
		exercisePlanTests(testFinalSort, referenceFinalSort);
	}
	...
	public void testGroupCustomersEmpty() throws Exception
	{
		...
		// set up the reference plan
		TableScanOperator referenceTableCustomerScan = new TableScanOperatorImpl(
				bufferPool, custTable.getResourceManager(), new LowLevelPredicate[] { pred }, customerColumns, 
				custTable.getResourceId(), 150);
	...
	public void testAggregateCustomersEmpty() throws Exception
	{
		...
		GroupByOperator referenceGroupBy = new GroupByOperatorImpl(referenceTableCustomerScan,
				groupCols, aggColIndices, aggs, aggTypes,
				groupOutput, aggOutput);
	...
	public void testMergeJoin() throws Exception
	{
		...
		TableScanOperator referenceTableCustomerScan = new TableScanOperatorImpl(
				bufferPool, customerTable.getResourceManager(), new LowLevelPredicate[0], customerColumns, customerTable.getResourceId(), 150);
		
		SortOperator referenceNationKeyCustomerTableSort = new SortOperatorImpl(referenceTableCustomerScan, 
				this.theInstance.getQueryHeap(), new DataType[] { customerTable.getSchema().getColumn(0).getDataType(), customerTable.getSchema().getColumn(3).getDataType() }, 
				(int) customerTable.getStatistics().getCardinality(), new int[]{ 1 }, new boolean[]{ true } );

		TableScanOperator referenceTableNationScan = new TableScanOperatorImpl(
				bufferPool, nationTable.getResourceManager(), new LowLevelPredicate[0], nationColumns, 
				nationTable.getResourceId(), 150);
		
		SortOperator referenceNationSort = new SortOperatorImpl(referenceTableNationScan, 
				this.theInstance.getQueryHeap(), new DataType[] { nationTable.getSchema().getColumn(0).getDataType(), nationTable.getSchema().getColumn(1).getDataType() }, 
				(int) nationTable.getStatistics().getCardinality(), new int[]{ 0 }, new boolean[]{ true } );

		// join nation and customer to get tuples of (customer_id, nation_name)
		MergeJoinOperator referenceCustomerNationMGJN = new MergeJoinOperatorImpl(
				referenceNationSort, referenceNationKeyCustomerTableSort, new int[] { 0 }, new int[] { 1 }, new int[] { -1, 1 }, new int[] { 0, -1 });

		SortOperator referenceCustomerNationJoinSort = new SortOperatorImpl(referenceCustomerNationMGJN, 
				this.theInstance.getQueryHeap(), new DataType[] { customerTable.getSchema().getColumn(0).getDataType(), nationTable.getSchema().getColumn(1).getDataType() }, 
				(int) ordersTable.getStatistics().getCardinality(), new int[]{ 0 }, new boolean[]{ true } );
		
		TableScanOperator referenceTableOrdersScan = new TableScanOperatorImpl(
				bufferPool, ordersTable.getResourceManager(), new LowLevelPredicate[0], ordersColumns, ordersTable.getResourceId(), 150);

		SortOperator referenceCustomerKeyOrdersTableSort = new SortOperatorImpl(referenceTableOrdersScan, 
				this.theInstance.getQueryHeap(), new DataType[] { ordersTable.getSchema().getColumn(1).getDataType(), ordersTable.getSchema().getColumn(3).getDataType() }, 
				(int) ordersTable.getStatistics().getCardinality(), new int[]{ 0 }, new boolean[]{ true } );
		
		// join orders to get tuples of (totalprice, customer_id, nation_name)
		MergeJoinOperator referenceCustomerNationOrdersMGJN = new MergeJoinOperatorImpl(
				referenceCustomerKeyOrdersTableSort, referenceCustomerNationJoinSort, new int[] { 0 }, new int[] { 0 }, new int[] { 1, -1, -1 }, new int[] { -1, 0, 1 });
		
		// sort by region
		SortOperator referenceCustomerNationOrdersJoinSort = new SortOperatorImpl(
				referenceCustomerNationOrdersMGJN, this.theInstance.getQueryHeap(), new DataType[] { ordersTable.getSchema().getColumn(3).getDataType(), customerTable.getSchema().getColumn(0).getDataType(), nationTable.getSchema().getColumn(1).getDataType() },
				(int) ordersTable.getStatistics().getCardinality(), new int[]{ 2 }, new boolean[]{ true } );
		
		// group totalprice per region		
		GroupByOperator referenceGroupBy = new GroupByOperatorImpl(
				referenceCustomerNationOrdersJoinSort, groupCols, aggColIndices, aggs, 
				aggTypes, new int[]{ 0, -1 } , new int[]{ -1, 0 });
	 ...
	 * @param referencePlan The plan of reference operators.
	 */
	...
	throws Exception
	{
		...
		List<DataTuple> referenceTuples = new ArrayList<DataTuple>();
