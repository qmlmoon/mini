...
public final class LayeredExtensionFactory extends AbstractExtensionFactory {
	 ...
	 */
	private void registerFactory(String clazz, boolean required) throws ExtensionInitFailedException {
		try {
			Class<? extends AbstractExtensionFactory> ec = Class.forName(clazz).asSubclass(AbstractExtensionFactory.class);
			this.factories.add(ec.newInstance());
		} catch (ClassNotFoundException cnfex) {
			if (required) {
				throw new ExtensionInitFailedException("The class '" + clazz + "' was not found. Check that it is in the class path.", cnfex);
			}
		} catch (ExceptionInInitializerError eiiex) {
			throw new ExtensionInitFailedException("The class '" + clazz + "' could not be loaded, because errors occurred in the initializers.", eiiex);
		} catch (ClassCastException ccex) {
			throw new ExtensionInitFailedException("The class '" + clazz + "' is not a subclass of '" + AbstractExtensionFactory.class.getName() + "'.", ccex);
		} catch (Throwable t) {
			throw new ExtensionInitFailedException("The class '" + clazz + "' could not be loaded due to an unknown error." + t.getMessage(), t);
		}
	}
