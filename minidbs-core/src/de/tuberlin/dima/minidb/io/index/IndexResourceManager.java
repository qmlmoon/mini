...
public class IndexResourceManager extends ResourceManager
{
	 ...
	 * 
	 * @param fileHandle The handle to the index's file.
	 * @param indexedTable The schema of the table that is indexed by this index.
	 * @throws IOException If the index file could not be accessed due to an I/O error.
	 * @throws PageFormatException If the index file did not contain a valid header.
	 */
	protected IndexResourceManager(RandomAccessFile fileHandle, TableSchema indexedTable)
	 ...
	 *  
	 * @param fileHandle The handle to the file which is used for the index.
	 * @param schema The schema for the new index.
	 * 
	 * @throws IOException If the index could not be created due to an I/O error.
	 */
	protected IndexResourceManager(RandomAccessFile fileHandle, IndexSchema schema)
	throws IOException
	{	
		// Open the channel. If anything fails, make sure we close it again
		try {
	 ...
	 * 
	 * @param newNumber The new page number of the first leaf page.
	 */
	public synchronized void updateRootPageNumber(int newNumber)
	throws IOException
	{
	 ...
	 * @param newNumber The new page number of the first leaf page.
	 */
	public synchronized void updateFirstLeafPageNumber(int newNumber)
	throws IOException
	{
		// check consistency with the schema
	...
	public CacheableData reserveNewPage(byte[] buffer, Enum<?> type) throws IOException, PageFormatException
	{
		...
		if (!(type instanceof BTreeIndexPageType)) {
			throw new IllegalArgumentException("Parameters must be a specifictaion of the page type through " +
					BTreeIndexPageType.class.getCanonicalName());
		}
	 ...
	 */
	@Override
	public BTreeIndexPage readPageFromResource(byte[] buffer, int pageNumber)
	throws IOException
	{
		// check that the page number is within range
		if (pageNumber < FIRST_DATA_PAGE || pageNumber > this.lastPageNumber) {
			throw new IOException("Page number " + pageNumber + " is not in valid range: [" +
	 ...
	 * 
	 * @param indexFile The file containing the index to be opened.
	 * @param indexedTable The schema of the table that is indexed by this index. 
	 * @return The IndexResourceManager to operate on the index file.
	 * @throws IOException Thrown, if an I/O error occurred.
	 * @throws PageFormatException Thrown, if the header of the index contained invalid data.
	 */
	public static IndexResourceManager openIndex(File indexFile, TableSchema indexedTable)
	throws IOException, PageFormatException
	{
		if (indexFile == null) {
		...
		try {
			...
			if (!indexFile.exists()) {
				throw new IOException("Table file '" + indexFile.getCanonicalPath() +
						"' does not exist.");
			}
			...
			("The user running the system has insufficient privileges for file manipulation.");
		}
	}
	
	/**
	 * Creates a new index with the given schema, indexing the a table whose schema is given
	 * by the index schema's
	 * {@link de.tuberlin.dima.minidb.catalogue.IndexSchema#getIndexTableSchema()} method.
	 * 
	 ...
	 * 
	 * @param indexFile The file to store the new index in.
	 * @param schema The schema of the index to create. 
	 * @throws IOException If an I/O problem occurred.
	 */
	public static IndexResourceManager createIndex(File indexFile, IndexSchema schema)
	throws IOException
	{
		if (indexFile == null) {
			throw new NullPointerException("Index file must not be null.");
	 ...
	 * 
	 * @param indexFile The file for the index to be deleted.
	 * @throws IOException If an I/O problem occurred.
	 */
	public static void deleteIndex(File indexFile)
	throws IOException
	{
		try {
			// check if the file exists
			if (!indexFile.exists()) {
				throw new IOException("Index file '" + indexFile.getCanonicalPath() +
						"' does not exist.");
			}
	 ...
	 * 
	 * @param channel The channel from which to read the index schema.
	 * @param tableSchema The schema of the table for which the index is created.
	 * @return The index schema read from the channel.
	 * @throws IOException Thrown if an error occurred during reading from the channel.
	 * @throws PageFormatException Thrown if the data read from the channel did not describe
	 *                             a valid index schema.
	 */
	private static IndexSchema readIndexHeader(FileChannel channel, TableSchema tableSchema)
	throws IOException, PageFormatException
	{
	 ...
	 * 
	 * @param schema The schema of the index that will be written to the header.
	 * @param channel The channel to write the data to.
	 * @throws IOException Thrown, if an error occurred during writing to the channel.
	 */
	private static void writeIndexHeader(IndexSchema schema, FileChannel channel)
	throws IOException
	{
	 ...
	 * @param buffer The buffer into which to read the bytes.
	 * @param num The number of bytes to read.
	 * 
	 * @throws IOException Thrown, when any I/O error occurred during reading.
	 */
	private static final void readIntoBuffer(FileChannel channel, ByteBuffer buffer, int num)
	throws IOException
	{
		// make space first
		buffer.clear();
	...
	/**
	 * Reads the given number of bytes from the given file channel into the given buffer. The bytes are read from the specified
	 * position. This method blocks until all bytes are read. The buffer will contain the new bytes from position
	 * 0 to position num.
	 * 
	 * @param channel The file channel to read from.
	 * @param buffer The buffer into which to read the bytes.
	 * @param position The position in the file channel from where to read the data.
	 * @param num The number of bytes to read.
	 * 
	 * @throws IOException Thrown, when any I/O error occurred during reading.
	 */
	private static final void readIntoBuffer(FileChannel channel, ByteBuffer buffer, long position, int num)
	throws IOException
	{
		// make space first
		buffer.clear();
	 ...
	 * @param channel The file channel to write to.
	 * @param buffer The buffer containing the data to be written.
	 * @param position The position in the file channel to write the data to.
	 * 
	 * @throws IOException Thrown, when any I/O error occurred during the writing.
	 */
	private static final void writeBuffer(FileChannel channel, ByteBuffer buffer, long position)
	throws IOException
	{		
		// write as long as we need to
		long bytes = buffer.remaining();
	 ...
	 * @param pageNumber The page number of the page where the buffer will be written to.
	 * @param pageSize The size of the page that will be written.
	 * @throws IOException Thrown, if an I/O problem occurred during the write operation.
	 */
