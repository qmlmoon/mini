...
public class TableResourceManager extends ResourceManager
{
	 ...
	 * 
	 * @param fileHandle The handle to the table's file.
	 * @throws IOException If the table file could not be accessed due to an I/O error.
	 * @throws PageFormatException If the table file did not contain a valid header.
	 */
	protected TableResourceManager(RandomAccessFile fileHandle)
	 ...
	 *  
	 * @param fileHandle The handle to the file which is used for the table.
	 * @param schema The schema for the new table.
	 * 
	 * @throws IOException If the table could not be created due to an I/O error.
	 */
	protected TableResourceManager(RandomAccessFile fileHandle, TableSchema schema)
	throws IOException
	{
		// Open the channel. If anything fails, make sure we close it again
		try {
	 ...
	 */
	@Override
	public final synchronized TablePage reserveNewPage(byte[] buffer)
	 ...
	 */
	@Override
	public final synchronized TablePage reserveNewPage(byte[] buffer, Enum<?> type)
	throws PageFormatException
	{
	...
	throws IOException
	{
		...
		if (Constants.DEBUG_CHECK) {
			...
			if (buffer.length < this.pageSize) {
				throw new IllegalArgumentException("The given buffer is too small for the specified page size ("
						+ this.pageSize + " bytes).");
			}
		}
		...
		if (Constants.DEBUG_CHECK) {
			...
			for (int i = 0; i < buffers.length; i++) {
				...
				if (buffers[i].length < this.pageSize) {
					throw new IllegalArgumentException("The given buffer is too small for the specified page size (" + this.pageSize + " bytes).");
				}
			}
		}
	 ...
	 */
	@Override
	public TablePage readPageFromResource(byte[] buffer, int pageNumber)
	throws IOException
	{
		if (Constants.DEBUG_CHECK) {
			// check that the page number is within range
	...
	public TablePage[] readPagesFromResource(byte[][] buffers, int firstPageNumber) throws IOException {
		...
		try {
			...
			throw new IOException("Page sequence [" + firstPageNumber + ", " + (firstPageNumber+buffers.length-1) + "] could not be read from table file.", ioex);
		}
	 ...
	 * 
	 * @param tableFile The file containing the table to be opened.
	 * @return The TableManager to operate on the table file.
	 * @throws IOException Thrown, if an I/O error occurred.
	 * @throws PageFormatException Thrown, if the header of the table contained invalid data.
	 */
	public static TableResourceManager openTable(File tableFile)
	throws IOException, PageFormatException
	{
		if (tableFile == null) {
		...
		try {
			...
			if (!tableFile.exists()) {
				throw new IOException("Table file '" + tableFile.getCanonicalPath() +
						"' does not exist.");
			}
	 ...
	 * otherwise be empty.
	 * 
	 * @param tableFile The file to store the new table in.
	 * @throws IOException If an I/O problem occurred.
	 */
	public static TableResourceManager createTable(File tableFile, TableSchema schema)
	throws IOException
	{
		if (tableFile == null) {
			throw new NullPointerException("Table file must not be null.");
	 ...
	 * 
	 * @param tableFile The file for the table to be deleted.
	 * @throws IOException If an I/O problem occurred.
	 */
	public static void deleteTable(File tableFile)
	throws IOException
	{
		try {
			// check if the file exists
	 ...
	 * 
	 * @param channel The channel from which to read the table schema.
	 ...
	 */
	private static TableSchema readTableHeader(FileChannel channel)
	throws IOException, PageFormatException
	{
		...
		for (int i = 0; i < numCols; i++) {
			...
			if (typeNum < 0 || typeNum >= BasicType.values().length) {
				throw new PageFormatException("Invalid data type given for column " + i +
						": " + typeNum);
			}
			...
			{
				throw new PageFormatException("Length for column " + i +
			...
			if (nameLength < 0 || nameLength >= Constants.MAX_COLUMN_NAME_LENGTH) {
				throw new PageFormatException("Length for column " + i +
						"'s name is out of bounds: " + nameLength);
			}
	 ...
	 * @param channel The channel to write the schema to.
	 * @throws IOException Thrown, if an error occurred during writing to the channel.
	 */
	private static void writeTableHeader(TableSchema schema, FileChannel channel)
	throws IOException
	{
	 ...
	 */
	private static void readIntoBuffer(FileChannel channel, ByteBuffer buffer, int num)
	throws IOException
	{
		// make space first
		buffer.clear();
